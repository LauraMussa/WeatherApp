...
export default function WeatherCard({ data }: { data: WeatherDataType | null }) {
  ...
  return (
    ...
    >
      {!data ? (
        <div className="flex flex-col justify-center items-center gap-4 min-h-[150px] px-4 md:px-0 text-center">
          <span className="text-lg md:text-xl">Introduce el nombre de una ciudad y obtén el pronóstico.</span>
          <p className="text-xs md:text-sm opacity-50 italic">
            Por ejemplo: Buenos Aires, Nueva York, París...
          </p>
        </div>
      ) : (
        <>
          <div className="flex flex-wrap items-center gap-4 mb-2">
            {statusIcon && React.cloneElement(statusIcon, { size: 40 })}
            <div className="flex-1 min-w-0">
              <h2 className="text-xl md:text-2xl font-bold leading-6 truncate">
                {data?.name}, {data?.sys.country}
              </h2>
              <p className="capitalize text-base mt-2 truncate">{data?.weather[0].description}</p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2 md:gap-4 mt-6 mb-3">
            ...
            {data?.main.feels_like !== undefined && (
              <span className="text-base md:text-lg opacity-80">{`${Math.round(
